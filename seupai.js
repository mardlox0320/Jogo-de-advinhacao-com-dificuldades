...
if (r && typeof r === 'object' && r.tentativas !== undefined) {
        lista.innerHTML += `<li>tentativas: ${r.tentativas} - numero advinhado: ${r.numero} - ${r.dificuldade}</li>`
    } else {
...
function gerarcustom1() {
    ...
    if(valor > 1000000000000) {
        alert("digite um numero menor do que 1 trilhâo");
        return
    }
...
const verificar = () => {
    ...
    if (val > generate) {
        ...
        if (h3) h3.remove()
        ovo = false
        savedRecordes.push({tentativas: points, numero: generate, dificuldade: dificuldade})
        localStorage.setItem(recordes, JSON.stringify(savedRecordes))
        lista.innerHTML += `<li>tentativas:${points} - numero advinhado:${generate} - ${dificuldade}</li>`
    }
}
const limpar = () => {
    lista.innerHTML = ''
